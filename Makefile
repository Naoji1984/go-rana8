...
deploy:
	...
	docker run --rm -v "$(PWD):/go-rana8" --volumes-from gcloud-config gcr.io/google.com/cloudsdktool/google-cloud-cli:455.0.0 gcloud run deploy $(SERVICE_ID) --project rana8-poc --region asia-northeast1 --platform managed --source /go-rana8/src
